<x-app-layout>
    <div class="container mx-auto py-8">
        ...
        <div class="flex justify-center">
            <h1
                class="text-4xl font-extrabold text-gray-800 mb-6 p-4 rounded-lg shadow-xl bg-blue-50 inline-block border-2">
                Lista de Reservas
            </h1>
        </div>
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-blue-600 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-base font-semibold uppercase tracking-wider">Cantidad de
                            Ubicaciones</th>
                ...
                <tbody class="bg-blue-50 divide-y divide-gray-200">
                    @forelse($reservations as $reservation)
                        <tr class="hover:bg-blue-100">
                            ...
                            <td class="px-6 py-4 text-center text-base text-gray-900">
                                {{ $reservation->status ? 'Activa' : 'Cancelada' }}</td>
                            <td class="px-6 py-4 text-center text-base text-gray-900">
                                {{ $reservation->location_quantity }}</td>
                            <!-- Mostrar el nombre del evento -->
                            <td class="px-6 py-4 text-center text-base text-gray-900">
                                {{ $reservation->event->name ?? 'Evento no disponible' }}</td>
                            <!-- Mostrar el nombre completo del usuario -->
                            <td class="px-6 py-4 text-center text-base text-gray-900">
                                {{ $reservation->user->name ?? 'Usuario no disponible' }}
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-gray-500">No hay reservas disponibles.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
