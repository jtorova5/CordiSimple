            ...
            <form action="{{ route('reservations.store') }}" method="POST" class="px-8 py-8">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="event_id" id="event_id"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-indigo-500"
                        value="{{ old('event_id') }}" placeholder="Escribe el ID del evento (opcional)">
                    @error('event_id')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="user_id" id="user_id"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-indigo-500"
                        value="{{ old('user_id') }}" placeholder="Escribe el ID del usuario (opcional)">
                    @error('user_id')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex justify-end">
                    ...
                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Crear
                        Reserva</button>
                </div>
            </form>
        </div>
